...
import { IdempotencyStore } from "../store/idempotency-store-redis.js";
  ...
  static async transfer(txtId, fromUserId, toUserId, amount) {
    try {
      ...
      if (!idempotencyResult.ok) {
        const { status } = idempotencyResult.record;
        if (status === IdempotencyEnum.PROCESSING) {
          return {
            success: false,
            code: "IDEMPOTENCY_IN_PROGRESS",
            message: `Transfer with txId ${txtId} is currently being processed.`,
          };
        }

        if (status === IdempotencyEnum.SUCCESS) {
          return {
            success: true,
            message: `Transfer with txId ${txtId} already processed.`,
            data: idempotencyResult.record.response.data,
          };
        }

        if (status === IdempotencyEnum.FAILED) {
          return {
            success: false,
            code: "IDEMPOTENCY_PREVIOUS_FAILURE",
            message: `Previous attempt for txId ${txtId} failed: ${idempotencyResult.record.errorMessage}`,
          };
        }
      }
      ...
      if (fromUserBalance === null) {
        await IdempotencyStore.saveFailure(
          txtId,
          `User with id ${fromUserId} not found`,
        );
        return {
          success: false,
      ...
      if (fromUserBalance < amount) {
        await IdempotencyStore.saveFailure(txtId, "Insufficient funds");
        return {
          success: false,
      ...
      if (toUserBalance === null) {
        await IdempotencyStore.saveFailure(
          txtId,
          `User with id ${toUserId} not found`,
        );
        return {
          success: false,
      ...
      try {
        ...
        console.error(
          `Error clearing idempotency key for ${txtId}: ${clearError.message}`,
        );
      }
      return {
        success: false,
